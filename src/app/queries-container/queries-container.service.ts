...
export class QueriesContainerService {
  ...
  public getMetadatumValuesWithMetadatum(installation: Installation, corpus: string, metadatum: Metadatum): Observable<any> {
    const endpoint = installation?.corpora.find(corp => corp.name === corpus)?.endpoint;
    const url = (environment.secureUrl ? HTTPS : HTTP) + endpoint;
    return this.http.get<any>(`${url}/metadatum-values/${corpus}/${STRUCT_DOC}.${metadatum.name}`)
      .pipe(map(res => {
